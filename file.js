...
// let r = pool.query("CREATE TABLE images (name text, imgoid oid);")

// let g = pool.query(`CREATE TABLE Users(Userid VARCHAR(455),Username VARCHAR(255) ,Name VARCHAR(255),Password VARCHAR(255),Imageid VARCHAR(455) );`)
    ...
    try {
      ...
      if (b.rowCount !== 0) {
        for (let j = 0; j <= b.rowCount - 1; j++) {
          ...
          let userd = await pool.query(`SELECT * FROM Users WHERE Username = $1`, [b.rows[j].owner])
...
app.get('/user-posts/:id', authenticateToken, async (req, res) => {
  ...
  if (check.rows[0].exists) {
    ...
    for (let i = userd.rowCount - 1; i >= 0; i--) {
      ...
      let cheak = await pool.query(`SELECT * FROM ${data}Posts WHERE Postid = $1`, [userd.rows[i].postid])
...
app.get('/home-posts/', authenticateToken, async (req, res) => {
  let auth = req.data
  let r = await users(auth.username)
  let info = []
  ...
  for (let i = userd.rowCount - 1; i >= 0; i--) {
    ...
    let cheek = await pool.query(`SELECT EXISTS(SELECT * FROM ${auth.username}bookmark WHERE  Postid = $1);`, [userd.rows[i].postid])
    let userad = await pool.query(`SELECT * FROM Users WHERE Username = $1`, [ userd.rows[i].owner])
...
app.post('/upload', authenticateToken, async (req, res) => {
  console.log(req.data)
  let g  = req.data
  const newTod = await pool.query(`UPDATE Users SET Imageid = $1  WHERE Username = ${g.userjake}`,[req.body.a]);
